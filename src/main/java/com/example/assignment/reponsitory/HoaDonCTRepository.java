...
public interface HoaDonCTRepository  extends JpaRepository<HoaDonChiTiet,Integer> {
    @Query( value = "select h from HoaDonChiTiet h where (:name is null or h.hoaDon.ma = :name) or (:name is null or h.hoaDon.nhanVien.ma = :name) or (:name is null or h.hoaDon.khachHang.ma = :name) " +
